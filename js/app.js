...
var allObstacles=[],
    ...
    price1,
    _datos,
    proofArrayEnemies= [enemy1, enemy2, enemy3],
    proofArrayPrices= [price1];
   
   //console.log(datos['fases'][0]['fase']);

//EXPLORACIÓN 2: INTRODUZCO LOS ENEMIGOS Y EL PREMIO CON UN BUCLE QUE AVANZA UNA GESTIÓN DESDE JSON
...
/* var price1= new Price ({
 ...
 for (var i=1; i<proofArrayEnemies.length+1; i++) {

    var enemigo= proofArrayEnemies[i];
     ...
     allObstacles.push(enemigo);
};


   for (var i = 0; i <  proofArrayPrices.length; i++) {
      var premio=proofArrayPrices[i];
...
/** Initialize arrays containing the bounds for checking obstacle collisions 
 ...
 var onDatosCargados= function(datos){

  _datos= datos;
   console.log(_datos['fases'][0]['fase']);
